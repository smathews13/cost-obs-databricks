...
SOURCE_TABLES = json.loads('["system.access.workspaces_latest","system.billing.list_prices","system.billing.usage","system.query.history"]')
TABLE_DEPENDENCIES = json.loads('{"daily_usage_summary":["system.billing.list_prices","system.billing.usage"],"daily_product_breakdown":["system.billing.list_prices","system.billing.usage"],"daily_workspace_breakdown":["system.access.workspaces_latest","system.billing.list_prices","system.billing.usage"],"sql_tool_attribution":["system.billing.list_prices","system.billing.usage","system.query.history"],"daily_query_stats":["system.query.history"],"dbsql_cost_per_query":["system.billing.list_prices","system.billing.usage","system.query.history"],"daily_tag_summary":["system.billing.list_prices","system.billing.usage"],"daily_tag_coverage_summary":["system.billing.list_prices","system.billing.usage"],"daily_apps_summary":["system.billing.list_prices","system.billing.usage"]}')
CURRENT_IDENTITY = spark.sql("SELECT current_user() AS principal").first()["principal"]
missing_access = []
for source_table in SOURCE_TABLES:
    try:
        spark.sql(f"SELECT * FROM {source_table} LIMIT 0").collect()
    except Exception as exc:
        missing_access.append((source_table, str(exc)))

if missing_access:
    display(
        spark.createDataFrame(
            missing_access,
            "source_table STRING, error STRING",
        )
    )
    principal = str(CURRENT_IDENTITY).replace("`", "``")
    schemas = sorted({".".join(table.split(".")[:2]) for table, _ in missing_access})
    grant_lines = ["-- Run as a metastore admin:"]
    grant_lines.append(f"GRANT USE CATALOG ON CATALOG system TO `{principal}`;")
    for schema_name in schemas:
        grant_lines.append(
            f"GRANT USE SCHEMA ON SCHEMA {schema_name} TO `{principal}`;"
        )
    for table_name, _ in missing_access:
        grant_lines.append(
            f"GRANT SELECT ON TABLE {table_name} TO `{principal}`;"
        )
    print("\n".join(grant_lines))
    print(
        "Continuing with partial capability. Aggregates that depend on inaccessible "
        "tables will be skipped."
    )
else:
    print(f"Permission preflight passed for {len(SOURCE_TABLES)} system tables.")
...
missing_source_tables = {table for table, _ in missing_access}
...
for table_name, template in TABLE_SQL.items():
    blocked_by = sorted(
        missing_source_tables.intersection(TABLE_DEPENDENCIES[table_name])
    )
    if blocked_by:
        build_results.append(
            (
                ...
                "SKIPPED_PERMISSION",
                0,
                "Missing SELECT on " + ", ".join(blocked_by),
            )
        )
        continue
...
skipped = [row for row in build_results if row[1] == "SKIPPED_PERMISSION"]
display(
    spark.createDataFrame(
        build_results,
...
if skipped:
    print(
        f"Skipped {len(skipped)} permission-dependent aggregates: "
