...
async def get_cur_status() -> dict[str, Any]:
    ...
    if _cur_status_cache["available"] is not None and (time.time() - _cur_status_cache["checked_at"]) < _CUR_STATUS_TTL:
        ...
        try:
            ...
            # A failed existence query is not evidence that the integration is
            # absent. Do not cache permission, network, timeout, or capacity errors.
            logger.warning("AWS CUR availability query failed transiently: %s", exc)
            raise
...
async def get_aws_actual_dashboard_bundle(
    ...
    summary = await get_aws_actual_summary(start_date, end_date)
    ...
    async def optional(name: str, call):
        async with semaphore:
            try:
                ...
                logger.warning("Optional AWS actual-cost section %s failed: %s", name, exc)
                return name, None, getattr(exc, "code", "QUERY_FAILED")

    outcomes = await asyncio.gather(
        optional("by_cluster", lambda: get_aws_costs_by_cluster(start_date, end_date)),
        optional(
            "by_charge_type",
            lambda: get_aws_costs_by_charge_type(start_date, end_date),
        ),
        optional("timeseries", lambda: get_aws_costs_timeseries(start_date, end_date)),
    )
    ...
    try:
        delta_cache_put(
            _dkey,
            "aws_actual/dashboard-bundle",
            _resp,
            ttl_seconds=60 if failures else cache_ttls.BUNDLE,
