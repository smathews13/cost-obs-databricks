...
| Cloud Costs | `billing/infra-bundle` · `aws-actual/dashboard-bundle` · `azure-actual/dashboard-bundle` · `gcp-actual/dashboard-bundle` · `billing/kpi-trend` | `app_response_cache` (estimates live) | `billing.usage` · `compute.clusters` · `access.workspaces_latest` | Estimates work without exports; rows missing instance metadata/pricing marked unavailable. Opt: AWS/Azure `actuals_gold`, GCP BigQuery-federated or curated Delta |
